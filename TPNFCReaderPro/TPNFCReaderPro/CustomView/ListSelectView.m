...
@interface ListSelectView()<UITableViewDelegate,UITableViewDataSource,TPSDeviceManagerDelegate>{
...
@property (nonatomic, strong) TPSDeviceManager *bleManager;
...
- (instancetype _Nonnull)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.bleManager = [[TPSDeviceManager alloc] initWithDelegate:self];
...
-(void)stopscanDevice
{
    [self.bleManager stopScanDevice];
    
}

#pragma mark - TPSDeviceManagerDelegate
...
- (void)initSelectView {
    ...
    self.selectView = [[UIView alloc]initWithFrame:CGRectMake(30, (MAX_HEIGHT-(self.collectionViewHeight+101))/2-80, MAX_WIDTH-60, self.collectionViewHeight+kSingleTitleHeight+kSingleBtnHeight+1)];
...
{
     CBPeripheral *selectedPeripheral = [_peripheralArray objectAtIndex:[indexPath row]];
    
    [self.bleManager connectPeripheral:selectedPeripheral];
